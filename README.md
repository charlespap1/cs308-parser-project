...
We decided to take on the challenging feature of making all our visual text change depending on the language specified. To do this, we specified all of our written labels to be bound to StringProperties. Thus, depending on the language specified, we used a LanguageHelper object to bind button text properties and visual object titles to the new language value for that item. As a result, we have many instance variables in our SetupScreen class to specify keys. In the future, we hope to find a way to use a smart data structure to cut down on this code. 

In addition, we also implemented the challenging feature of allowing the user to redo and undo previous commands. We implemented this by storing a history of programs with states that we could move between in the appropriate direction when the correct button is pressed.

### Impressions

In the beginning of this assignment, we had a great flow of work and data between the front and back ends through the Controller. However, the new Complete objectives required much more information to travel between the two ends. As a result, our Controller had to accommodate and now contains a fat, kind of smelly method to set all of our front end buttons to functions in the backend.  In the future, it would be great to find a way to cut down on all of this deliberate setting so that our Controller could once again function as a streamlined network between front and back. Additionally, we ran into this problem of trying to do too much with our SetupScreen class. We broke it up into three different classes to deal with setup, but if we would have known the extent to the complete in the beginning, perhaps we could have been more deliberate in our construction. We might have further democratized objects which interacted more frequently together to form a more streamlined hierarchy. This was not a flaw in the assignment, but it is something we will definitely be thinking about in the future when we plan out our next project. 

In the future, it might be nice to have the lab on APIs and reflection before we start the project so that we have a better understanding going in. This way, we could develop a more strategic plan. While constructing our initial plan, we did not have a great grasp on the idea of APIs. As a result, we were not able to come up with a helpful, applicable internal frontend API. Knowing right off the bat how to use reflection would have been helpful in minimizing API changes and overall design structure changes throughout the project. 

In terms of backend design, the addition of the token interface structure was very helpful in terms of having a standardized format that all classes had to follow. This made it very easy when taking in parameters to handle any type rather than requiring numerous if statements to compare instanceof. The addition of new instructions or token types is now very easy and follows a format for what methods must be implemented and the way in which these instructions interact with the turtlemaster.

Future additions to this project might include the adding of the recursive or grouping functionality. In order to add recursive, we would need to slightly modify the way our parser handles new commands. In order to implement grouping, it might depend slightly on group’s interpretation of how certain commands handle multiple inputs, but could generally follow a recursive format where the instruction would call itself again with a subset of its parameters. For example, fd (10,20,30,40) should move forward 10 20 30 and then 40. This could be achieved by executing fd on each of its arguments. In making the recursive calls, it could simply remove one of its parameters each time. In a different case, Sum (10,20,30,40) would have to re-add the result of its parameter 10 20 to the list of parameters when making the next recursive call. These are easy modifications to make, and would just require handling groupings differently in different abstract subclasses of Instruction. Instructions of some type might re-add the params, while others might just remove them. That being said, the total functionality of our project meets much of the specification and is relatively comprehensive in terms of handling errors and interesting use-cases.

Overall, the project was very interesting in terms of making design choices to create easily extendable and reusable code through the use of Reflection, API’s, Interfaces and Lambdas. We were able to take what was learned in class over the course of this project and find ways to apply it to improve our code.
